...
document.addEventListener("DOMContentLoaded", function() {
  const table = $('#myTable').DataTable({
    ...
    autoWidth: false,             // let column widths respect resizing
    fixedColumns: {
        leftColumns: 2            // first 2 columns stay fixed
    },
    paging: false,
    searching: true,
    ordering: true,
  ...
  $('#myTable thead th').resizable({
    handles: 'e',   // drag right edge
    stop: function () {
      table.columns.adjust(); // recalc widths after resizing
    }
  });
});
